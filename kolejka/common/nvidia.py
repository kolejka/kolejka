...
class NvidiaSMILog:
    class GPULog:
        def __init__(self, element=None):
            ...
            self.power_draw = 0.0
            power_readings = element.find('power_readings') or element.find('gpu_power_readings')
            if power_readings:
                power_draw = power_readings.find('power_draw') or power_readings.find('instant_power_draw')
                if power_draw:
                    self.power_draw = float(power_draw.text.split()[0])
